...
int main(int argc, char **argv) {
  if (argc < 7) {
    std::cerr << "[ERROR] usage: "
              << " ./ocr_db_crnn_demo det_model_file rec_model_file cls_model_file "
                 "image_path charactor_dict config [output_image_path]\n";
    std::cerr << "Example: " << argv[0] << " det_model rec_model cls_model image.jpg dict.txt config.txt [output.jpg]\n";
    std::cerr << "Example (no output image): " << argv[0] << " det_model rec_model cls_model image.jpg dict.txt config.txt\n";
    exit(1);
  }
